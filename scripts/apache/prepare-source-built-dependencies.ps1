...
$archiveUri = "https://downloads.apache.org/apr/apr-iconv-$AprIconvVersion-win32-src.zip"
...
$aprCompatReleaseRoot = if ($Arch -eq 'x64') {
    Join-Path $aprCompatRoot 'x64\Release'
} else {
    Join-Path $aprCompatRoot 'Release'
}
$apriconvBuildRoot = if ($Arch -eq 'x64') {
